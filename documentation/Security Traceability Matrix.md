...
| Test suite | `backend/tests/test_security.py` — **785** cases, all passing, alongside `backend/tests/firestore_rules_emulator_check.py` for V10 and `frontend/src/services/api.test.ts` (93 cases) for the V3 client half and H8; the counts grow as tests are added, so treat "all pass" rather than the number as the criterion |
...
Expect every test to pass — **785** cases. The classes named below are the test classes in that
...
| Authenticated cross-tenant read and write | A caller authenticated as one user listed and created workbooks owned by another. `current_user` is referenced zero times in all five handler bodies | **Confirmed open, not fixed.** This is the accepted residual §0.8.3.3 names and D13 explains: scoping requires the excluded domain service layer. It is the top follow-up | residual 1, F2, D13, SECURITY.md item 1 |
