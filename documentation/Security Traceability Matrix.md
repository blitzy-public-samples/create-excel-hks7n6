...
| Verified how | `terraform fmt -check` on `main.tf` + `variables.tf` (clean; `terraform validate` is blocked by the pre-existing `outputs.tf`), `bash -n scripts/deploy.sh`, a YAML parse of `ci.yml`, `pyflakes` on every changed module, the `pytest` suite below, and the Firestore emulator check |
| Test suite | `backend/tests/test_security.py` — **529** cases, all passing, alongside `backend/tests/firestore_rules_emulator_check.py` for V10 and `frontend/src/services/api.test.ts` (41 cases) for the V3 client half and H8; the counts grow as tests are added, so treat "all pass" rather than the number as the criterion |
...
Expect every test to pass — **529** cases. The classes named below are the test classes in that
...
| 12 | `backend/requirements.txt` | CREATE | V3, V6, V9, A06. `slowapi` stays pinned as the plan-named package although nothing imports it (R32, D52 superseded) |
...
| 25 | `.github/workflows/ci.yml` | UPDATE | A06 — the dependency-audit gate, whose fourteen suppressions are justified inline, one per advisory, rather than carried as an opaque baseline list (R52); the Firestore emulator job that turns the committed rules from an asserted artifact into an executed gate (R45); the frontend interceptor step with `continue-on-error` removed and the seven undeclared packages installed at pinned versions (R40); the frontend audit reduced to a self-promoting three-state check that becomes a real gate by itself once the manifest is fixed (R53); and the step that gates the four database-seam classes, so a regression in the authenticated query fails the build rather than the suite merely still passing |
...
| `frontend/src/services/auth.ts` (sign-in flow) | The flow is a contract that must not change — the Firebase sign-in is the front door, which is why V3 bridges into `get_current_user` rather than replacing the identity system. The same `signInWithEmailAndPassword`, `signOut`, lazy `getAuth()` and error handling remain; only the unresolvable type import and the double cast changed (row 37). |
...
| `infrastructure/terraform/*.tf` | CI asserts the presence of specific arguments and resources. `terraform validate` is not run | It fails on HEAD for a pre-existing reason: `outputs.tf` references seven resources no configuration declares. `main.tf` and `variables.tf` validate clean in isolation |
