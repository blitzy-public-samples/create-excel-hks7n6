...
| Cloud Function invoker identity | `scripts/deploy.sh`, `infrastructure/terraform/main.tf` | `--allow-unauthenticated` is not used. Terraform declares the invoker membership **authoritatively**, so any member not listed is removed on every apply — that apply is the only thing that changes the policy. The deployment script does not mutate it; it **reads and asserts** it in the read-only preflight, ahead of the first mutation, and aborts if `allUsers` or `allAuthenticatedUsers` holds the invoker role, if the intended service account does not hold it, or if the policy cannot be read at all. The last case is the one worth stating: an unreadable policy is never treated as proof that no public binding exists. The function also runs as its own service account holding no role, instead of the App Engine default account that carries project Editor. |
...
**823 tests, all passing** as measured by the command above. They assert the `401` on all five routes, rejection of a forged token,
